...
- OCR 识别记录：每次识别成功会保存到本地记录，后续创建错题时可以直接导入。
...
- 本地开发阶段需要在微信开发者工具中关闭“校验合法域名、web-view、TLS 版本以及 HTTPS 证书”。项目私有配置 `project.private.config.json` 中已将 `urlCheck` 设为 `false`。
...
4. 编译运行。
5. 在开发者工具“详情 -> 本地设置”中确认已关闭“校验合法域名、web-view、TLS 版本以及 HTTPS 证书”。
6. 从首页进入“开始识别”或“进入错题本”。
...
本地模拟器或真机调试时，应使用电脑当前局域网 IP。当前代码中使用的是 `10.29.45.6`，如果网络变化，需要重新查看电脑 IP 并修改这里。

后端已配置为监听 `0.0.0.0:5000`，这样局域网内设备才能访问 Flask 服务。启动后终端应出现类似：
...
- 新增错题。
- OCR 结果转错题。
- 从记录导入题目、答案或解析。记录来源包含已保存笔记和 OCR 识别结果。
- 直接拍照识别题目、答案或解析。
- 编辑错题。
- 删除错题。
- 查看详情。
...
- 从记录创建：进入后页面标题显示“从记录创建”，题目、答案、解析字段旁只显示“从记录填入”。记录来源包含已保存笔记和 OCR 识别结果。
- 拍照创建：进入后页面标题显示“拍照创建”，会先识别题目；题目、答案、解析字段旁只显示“拍照识别填入”。
- 手动创建：进入后页面标题显示“手动创建”，不显示导入或拍照按钮，只保留手动输入表单。

相关页面：

```text
miniprogram/pages/mistakes/
miniprogram/pages/mistake-editor/
```

记录导入使用方式：

1. 先保存一些笔记，或在“图片识别”页面完成一次 OCR。
2. OCR 识别成功后，文本会自动保存为本地识别记录。
3. 进入“错题本”页面，点击“从记录创建”。
4. 在题目、答案或解析下方点击“从记录填入”。
5. 选择一条笔记记录或 OCR 识别记录后，会自动填入对应字段。
6. 如果没有任何记录，页面会提示先保存笔记或去“图片识别”。
...
- [ ] OCR 结果可以保存为笔记。
- [ ] OCR 结果可以保存为错题。
- [ ] 新增错题时可以从记录导入题目、答案或解析。
- [ ] 新增错题时可以分别拍照识别题目、答案或解析。
- [ ] 笔记可以搜索、筛选、编辑、删除。
- [ ] 错题可以搜索、筛选、编辑、删除。
- [ ] 真机调试可以访问后端。
- [ ] 后端已部署为 HTTPS。
